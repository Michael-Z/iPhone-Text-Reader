...
// Work around duplicate definition ...
#define Byte Byte2
    ...
    if (!GetUncompressedRecord
        ...
        for (i = 0, ptr = buf + 10; i < nsubrecords; i++) {
            ...
            if (typecode == PLKR_MDTYPE_DEFAULTCHARSET) {
                ...
                newdoc->author = _plkr_strndup (ptr + 4, subrecord_length);
                ptr += (4 + subrecord_length);

            }
            else if (typecode == PLKR_MDTYPE_TITLE) {

                newdoc->title = _plkr_strndup (ptr + 4, subrecord_length);
                ptr += (4 + subrecord_length);
...
// #endif                          /* def DEBUGURLS */
    ...
    urls = (unsigned char **) malloc (nurls * sizeof (unsigned char *));
    memset (urls, 0, nurls * sizeof (unsigned char *));
    ...
    )
{
    ...
    if (strncmp ((unsigned char *) (utilbuf + 60), id_stamp, 8) != 0) {
    ...
    newdoc->name = _plkr_strndup (utilbuf, MIN (strlen (utilbuf), 32));
...
// JIMB BUG BUG - we need to handle this properly!!!
//         unsigned char *owner_id = plkr_GetConfigString (NULL, "owner_id", NULL);
// 
//         if (owner_id != NULL) {
//             unsigned long crc;
    ...
    )
{
    ...
    result =
        ...
        pdbread (/*(int)*/ (handle->dbprivate), buffer,
    ...
    )
{
    ...
    // if (fstat (fp, &buf) != 0) {
    if (fstat ((FILE*)(handle->dbprivate), &buf) != 0) {
...
unsigned char* plkr_GetRecordURL
    (
    plkr_Document * doc,
    int record_index
    )
