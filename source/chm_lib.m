                              ...
                              Int64 len)
{
    ...
    /* NOTE: this might be better done with CreateFileMapping, et cetera... */
    {
        ...
        origOffsetLo = SetFilePointer(h->fd, 0, (long *)&origOffsetHi, FILE_CURRENT);
        offsetLo = SetFilePointer(h->fd, offsetLo, (long *)&offsetHi, FILE_BEGIN);
        ...
        SetFilePointer(h->fd, origOffsetLo, (long *)&origOffsetHi, FILE_BEGIN);
    }
#else
#ifdef CHM_USE_PREAD
#ifdef CHM_USE_IO64
        ...
        for (i = blockAlign; i > 0; i--)
        {
            ...
            if (h->lzx_last_block != (Int32)curBlockIdx)
            {
                ...
                if (!_chm_get_cmpblock_bounds(h, curBlockIdx, &cmpStart, &cmpLen) ||
                    cmpLen < 0                                                    ||
                    cmpLen > (Int32)h->reset_table.block_len + 6144                      ||
